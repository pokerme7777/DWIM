...
def find_in_image(image: Image.Image, object_name: str) -> list[ImagePatch]:
    ...
    output_box = [ImagePatch.from_bounding_box(image, box) for box in box_records]
    number_of_box = len(output_box)

    #feedback
    if number_of_box<1:
        print(f'No {object_name} has been found.')
    elif number_of_box==1:
        print(f'Only one {object_name} has been found.')
    else:
        print(f'There are {number_of_box} {object_name} that have been found.')
    return output_box
...
def verify_property(image_patch: Image.Image, object_name: str, property: str) -> bool:
    try:
        ...
        output_yes_no = module.verify_property(
        ...
        question = template.format(object_name=object_name, property=property)

        print(f"Answer of '{question}': {output_yes_no}.")
        return output_yes_no


def best_description_from_options(image: Image.Image, object_name: str, property_list: list) -> str:
    module = ModuleProvider().get_module("simple_query")
    assert module is not None, "Module not found"
    output_property = module.best_description_from_options(image=image, object_name=object_name, property_list=property_list)
    print(f'Best description from options about {object_name}: {output_property}.')
...
def simple_query(image_patch: Image.Image, question: Optional[str]) -> str:
    ...
    output_answer = module(image_patch, question)
    print(f"Answer of '{question}': {output_answer}.")
...
def complex_query(image: Image.Image, question: str) -> str:
    module = ModuleProvider().get_module("simple_query")
    assert module is not None, "Module not found"
    output_answer = module.complex_query(image=image, question=question)
    print(f'Answer of {question}: {output_answer}.')
    return output_answer
...
def captioning(image: Image.Image, question: str) -> str:
    ...
    print(f"Image caption: {caption_output}.")
...
def llm_query(question: str) -> str:
    ...
    assert module is not None, "Module not found"
    print(f'Answer of {question}: {module(question)}.')
