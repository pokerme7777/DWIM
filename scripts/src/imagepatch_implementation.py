...
class ImagePatch:
    ...
    def llm_query(self, question: str, long_answer: bool = True) -> str:
        """Answers a text question using GPT-3. The input question is always a formatted
        string with a variable in it.

        Parameters
        ----------
        question: str
            the text question to ask. Must not contain any reference to 'the image' or
            'the photo', etc.
        long_answer: bool
            whether to return a short answer or a long answer. Short answers are one or
            at most two words, very concise. Long answers are longer, and may be
            paragraphs and explanations. Defalt is True (so long answer).
...
def best_image_match(
    ...
    """
    return best_image_match_impl(list_patches, content, return_index)
...
def distance(patch_a: ImagePatch, patch_b: ImagePatch) -> float:
    ...
    """
    return distance_impl(patch_a, patch_b)
...
def find_in_image(image_patch: Image.Image, object_name: str) -> list[ImagePatch]:
