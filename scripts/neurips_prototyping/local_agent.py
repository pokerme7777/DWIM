...
class MistralVllmMReactAgent:
    ...
    # original version  nocaptioning
    # def reset_ori(self, query: str):
    #     prompt = self.prompter(query)
    #     self.state = [{"role": "user", "content": prompt}]

    def reset(self, query: str, caption_observation:str):
        prompt = self.prompter(query, caption_observation)
        self.state = [{"role": "user", "content": prompt}]


class MistralVllmMReactAgentWithPinnedEngine:
    def __init__(
        self,
        prompter: Callable[[str], str],
        observation_renderer: Callable[[Observation], str],
...
if __name__ == "__main__":
    ...
    agent = MistralVllmMReactAgent(prompter, observation_renderer)
    ...
    action = agent.act()
    print(action)
    action = """<code>patch = ImagePatch(image)\nprint(patch.complex_query("Could you please describe this image as detailed as possible?"))</code>"""
