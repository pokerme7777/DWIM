...
class Llama31VllmMReactAgentWithPinnedEngine:
    def __init__(
        ...
        self.llm = llm_constructor.remote(model=self.model, enable_lora=True, **vllm_engine_kwargs)  # change enable_lora=True
    ...
    def _do_local_text_generation(self, input_text: str) -> str:
        vllm_output = ray.get(
            self.llm.generate.remote([input_text], self.sampling_params, lora_request=LoRARequest(self.lora_folder))
        )
        generated_text = vllm_output[0].outputs[0].text
        return generated_text
...
if __name__ == "__main__":
    # for higher version of vllm: worker_use_ray=True need to be set distributed-executor-backend=ray
